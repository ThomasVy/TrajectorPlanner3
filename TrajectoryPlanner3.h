...
using namespace std;
using namespace cv;
const Vec3b WALL(0,0,255); //B,G,R
const Vec3b EMPTY_SPACE(150,0,0);
int RESOLUTION = 0; //size of the robot
//fixing the resolution issue //works good
class Position{
	private:
		float cost;
		float total_cost;
					...
					if(j!=0 || i!=0)
					{
						...
						if(newPoint.x>=0 && newPoint.x<walls.rows && newPoint.y>=0 &&newPoint.y < walls.cols)
						{
							...
							if(space != WALL)
							{
								...
								float neighbourTotalCost = Position::distanceToGoal(newPoint, goal) + new_cost + space[1]/25;
	...
	private:
		...
		void dilation(Wall & wall) //need to check direction now
		{
			if(wall.direction[0] == true) //checks if its an outside wall block
			{
				...
				{
					if(wall.x+i >=0 && wall.x+i<image.rows)
					{
						for(int j =-RESOLUTION; j<=RESOLUTION;j++)
						{
							if(wall.y+j >= 0 && wall.y+j <image.cols)
							{
	...
	public:
		...
		{ 
			...
			for(int i =0; i< image.rows;i++)
			{
				...
				{ 
					...
					if(colour)
						convertedImage.at<Vec3b>(i,j)=EMPTY_SPACE;
					else
					{
						convertedImage.at<Vec3b>(i,j)=WALL;
					}
				}
			}
		...
		void insert_borders ()
		{
			...
			for(int i =0; i< image.rows;i++)
			{
				for (int j =0; j< image.cols;j++)
				{ 
					if(convertedImage.at<Vec3b>(i,j) ==WALL)
					{
		...
		bool * checkSpace (int i, int j)
		{
			...
			{
				if(convertedImage.at<Vec3b>(i+1,j) == EMPTY_SPACE)//right
				{
					direct[0] =true;
					direct[1] =true;
				}
				if(j+1<convertedImage.cols && convertedImage.at<Vec3b>(i+1,j+1) == EMPTY_SPACE)//top right
				{
					direct[0] =true;
					direct[5] =true;
				}
					if(j-1 >=0 && convertedImage.at<Vec3b>(i+1,j-1) == EMPTY_SPACE)//bottom right
				{
					direct[0] =true;
					direct[7] =true;
				}
			...
			if(i-1>=0)
			{
				if(convertedImage.at<Vec3b>(i-1,j) == EMPTY_SPACE)//left
				{
					direct[0] =true;
					direct[2] =true;
				}
				if(j+1<convertedImage.cols && convertedImage.at<Vec3b>(i-1, j+1) == EMPTY_SPACE)//top left
				{
					direct[0] =true;
					direct[6] =true;
				}
				if(j-1 >=0 && convertedImage.at<Vec3b>(i-1, j-1) == EMPTY_SPACE)//bottom left
				{
					direct[0] =true;
					direct[8] =true;
				}
			}
			if(convertedImage.at<Vec3b>(i,j+1) == EMPTY_SPACE)//up
			{
				direct[0] =true;
				direct[3] = true;
			}
			if(convertedImage.at<Vec3b>(i,j-1) == EMPTY_SPACE)//down
			{
				direct[0] =true;
				direct[4] = true;
			}
		...
		void planner (Point & start, Point & goal)
		{
			...
			while(Position::distanceToGoal(currentPoint, goal)>1){
				...
				{
					space.at<Vec3b>(currentPoint.x, currentPoint.y)= Vec3b((int)closedList.back()->getCost(),200, 0);
			...
			while(it != finalTrail.end())
			{
				...
				image.at<uchar>(point1.x, point1.y)= 100;
				it++;
			}
